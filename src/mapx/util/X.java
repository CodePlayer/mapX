...
public class X {
	...
	// 此类是通用工具类，全为静态方法，不需要实例化
	private X() {}
	/**
	 * 跳转到用户自定义页面的result标识变量
	 */
	public final static String CUSTOM = "custom"; // 用户自定义
	/**
	 * 跳转到INPUT页面的result标识变量
	 */
	public final static String INPUT = "input"; // 跳转到输入页面
	/**
	 * 直接输出Ajax信息的result标识变量
	 */
	public final static String AJAX = null; // 输出Ajax信息
	/**
	 * 跳转到系统公共错误页面的result标识变量
	 */
	public final static String ERROR = "global_error"; // 跳转到错误页面
	/**
	 * 跳转到系统公共的用于文件下载的result标识变量
	 */
	public final static String DOWNLOAD = "global_download";
	 ...
	 * @param obj 指定的对象
	 * @return
	 * @see mapx.util.ArrayUtil#isArray(Object)
	 */
	...
	/**
	 * 获取PrintWriter对象，用于Ajax输出，并进行Ajax输出相应设置。<br>
	 * 此方法在一次请求执行中最好只调用一次，否则会重复设置Ajax输出环境(没有其他影响，只是重复设置造成不必要的性能消耗。)<br>
	 * Ajax设置如下：设置Struts不再执行处理result，设置输出内容类型为"text/html;charset=utf-8"
	 * @return
	 */
	public static PrintWriter getPrintWriter() {
		ActionContext.getContext().getActionInvocation().getProxy().setExecuteResult(false);
		return HttpUtil.getWriter();
	}

	/**
	 * 执行性能测试
	 * @param testMethod 测试方法1接口实现类
	 * @param testMethod2 测试方法2接口实现类
	 * @param count 循环执行次数
	 */
	public static void testPerformance(TestMethod testMethod, TestMethod testMethod2, int count) {
		System.err.println("==========启动性能测试（单位：ms）==========");
		...
		for (int i = 0; i < count; i++) {
			testMethod2.runMethod();
		}
		long t3 = System.currentTimeMillis();
		System.out.println("方法1开始时间：" + t1);
		System.out.println("方法1结束时间：" + t2);
		long diffTime = t2 - t1;
		System.out.println("方法1执行时间：" + diffTime);
		System.out.println("===============================");
		System.out.println("方法2开始时间：" + t2);
		System.out.println("方法2结束时间：" + t3);
		long diffTime2 = t3 - t2;
		System.out.println("方法2执行时间：" + diffTime2);
		System.out.println("===============================");
		System.out.println("方法1执行时间-方法2执行时间=" + (diffTime - diffTime2));
		System.err.println("==========性能测试完毕==========");
	}
}
